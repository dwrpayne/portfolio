...
class RbcRawActivity(BaseRawActivity):
    ...
    def CreateActivity(self):
        ...
        if self.type == 'Dividend' and float(self.net_amount) < 0 and self.qty:
            self.type = 'Buy'
        ...
        if 'EXCHANGE RATE' in self.description:
            ...
            settle_amount = self.net_amount
            self.net_amount /= rate

            Activity.objects.create_fx(to_currency=self.currency, to_amount=-self.net_amount,
                                       from_currency=settle_currency, from_amount=settle_amount,
                                       account=self.account, trade_date=self.day, security=security,
